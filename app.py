...
def process_frames():
    global processed_frame, last_processed_time, current_face_status, movement_state, previous_face_data
    ...
    while True:
        if not frame_queue.empty():
            ...
            # Determine movement state if a face is detected
            if len(faces) > 0:
                (x, y, w, h) = faces[0]  # Take the first detected face for simplicity
                center_x = x + w // 2
                center_y = y + h // 2
                
                with movement_lock:
                    movement_state = calculate_movement((center_x, center_y, w, h), frame.shape[1], frame.shape[0])
                    previous_face_data = (center_x, center_y, w, h)
            else:
                with movement_lock:
                    movement_state = "stop"
                    previous_face_data = None
            ...
            # Encode the processed frame to JPEG format for streaming purposes
            _, buffer = cv2.imencode('.jpg', frame)
